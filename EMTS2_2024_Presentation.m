...
function interactive_trace_plot()
    ...
    title(sprintf('Radar Trace Data (%s)',radarFN));
%     subtitle(radarFN)
...
%             waveformDist = d2(1:nr);
    ...
    function createNewFigure(~, ~)
        ...
        ax1_new = subplot(1,3,1:2);
        ax2_new = subplot(1,3,3);
        ...
        [Z,Zp,x,waveformDist]=procJPLradar2(D,sfile,mrho,dsmooth,thresh,crange); % load and process data with skycal
        traceMatrixSubset = Zp(:,closeIndices);
        imagesc(ax1_new, traceMatrixSubset,[0 handles.newMax]); colorbar;  % Optional, adds a color bar to indicate the color scale
        title(ax1_new, 'Subset of Radar Trace Data');
        xlabel(ax1_new, 'Trace Number'); ylabel(ax1_new, 'Sample Index');
        axis tight;
        ...
        % Generate an array of positions to place x-lines
        ixLowArr = ixLowMin:2:ixLowMax;
        hold(ax1_new, 'on');  % Ensure that the image is not erased
        delete(findall(ax1_new, 'Type', 'ConstantLine'));  % Remove existing lines
        % Plot new x-lines for each position in ixLowArr
        arrayfun(@(x) xline(ax1_new, x, 'r'), ixLowArr);
        hold(ax1_new, 'off');
        ...
        firstLocation = traceMatrixSubset(:,ixLowArr);
        M1 = median(firstLocation, 2);  % Median of selected traces
        hMedian = plot(ax2_new, M1, dist, 'k-', 'LineWidth', 3);  % Redraw the plot
        set(ax2_new, 'YDir', 'reverse', 'LineWidth', 2, 'FontSize', 14);
        ylim(ax2_new, [dist(displayLow) dist(displayHigh)]);
        title(ax2_new, 'Selected Radar Traces and Measured Snow Depth');
        xlabel(ax2_new, 'Power spectral density');
        ylabel(ax2_new, ['Distance in snow [m] using $\rho=' sprintf('%.0f', mrho) '[kg/m^3]$'], 'Interpreter', 'latex');
        % Drawing a horizontal lines
        hline_surface = line(ax2_new, xlim(ax2_new), [dist(yIndMin) dist(yIndMin)], 'Color', 'green', 'LineStyle', '-', 'LineWidth', 2);
        hline_ground = line(ax2_new, xlim(ax2_new), [dist(yIndMax) dist(yIndMax)], 'Color', 'blue', 'LineStyle', '-', 'LineWidth', 2);
        hline_pitDepth = line(ax2_new, xlim(ax2_new), [dist(nearestIndex) dist(nearestIndex)], 'Color', 'blue', 'LineStyle', '--', 'LineWidth', 2);
        legend(ax2_new, [hMedian, hline_surface,hline_ground, hline_pitDepth], {'Median of Radar Traces', 'Radar Surface Return', 'Radar Ground Return' , 'Measured Pit Depth'}, 'Location', 'northeast');
        ...
        annotation('textbox', [0.001 0.75 0.15 0.05], 'String', get(annotationHandles.id, 'String'), ...
                   'EdgeColor', 'none', 'FontSize', 10, 'HorizontalAlignment', 'left');
        annotation('textbox', [0.001 0.7 0.15 0.05], 'String', get(annotationHandles.height, 'String'), ...
                   'EdgeColor', 'none', 'FontSize', 10, 'HorizontalAlignment', 'left');
        annotation('textbox', [0.001 0.65 0.15 0.05], 'String', get(annotationHandles.density, 'String'), ...
                   'EdgeColor', 'none', 'FontSize', 10, 'HorizontalAlignment', 'left');
        annotation('textbox', [0.001 0.6 0.15 0.05], 'String', get(annotationHandles.temp, 'String'), ...
                   'EdgeColor', 'none', 'FontSize', 10, 'HorizontalAlignment', 'left');
        annotation('textbox', [0.001 0.55 0.15 0.05], 'String', get(annotationHandles.velocity, 'String'), ...
                   'EdgeColor', 'none', 'FontSize', 10, 'HorizontalAlignment', 'left');
        annotation('textbox', [0.001 0.5 0.15 0.05], 'String', get(annotationHandles.res, 'String'), ...
                   'EdgeColor', 'none', 'FontSize', 10, 'HorizontalAlignment', 'left');

        annotation('textbox', [0.83 0.18 0.15 0.05], 'String', get(annotationHandles.height2, 'String'), ...
                   'EdgeColor', 'none', 'FontSize', 10, 'HorizontalAlignment', 'right');
        annotation('textbox', [0.83 0.12 0.15 0.05], 'String', get(annotationHandles.hYCoordText, 'String'), ...
                   'EdgeColor', 'none', 'FontSize', 10, 'HorizontalAlignment', 'right');
    end
