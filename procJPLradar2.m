...
function [Z,Zp,x,dist]=procJPLradar2(dfile,skyfile,mrho,dsmooth,thresh,crange)
if nargin<6
...
Z=dfile; % load radar data file
% Z=D'; % transpose due to collection in row vectors
[nr,nc]=size(Z); % get size
x=1:nc; %dist=d(1:nr); % make vectors for plot
es=e_snowdry(mrho,fhigh,-5); % get diel const for given density (mrho=0 for air)
vsnow=v./sqrt(real(es)); % snow velocity
dtemp=0.5*w*Tpl/(BW)*vsnow; % distance in snow
dist = dtemp(1:nr);
